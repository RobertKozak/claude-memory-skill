...
**On session start:**
- If no project memory file exists for the current directory, one is created automatically with three sections: `## Project` (directory name), `## User` (name from profile if known), and `## Session` (datetime stamp)
- All saved memory is injected into context so Claude is immediately aware of past work
- Greet the user by name if their name is saved in `profile.md`
- Reference the last thing worked on from project memory
...
**On session end:**
- A summary of what happened (user messages, tools used, next actions) is always written to `~/.claude/memory/SESSION_NOTES.md` as a new timestamped entry
